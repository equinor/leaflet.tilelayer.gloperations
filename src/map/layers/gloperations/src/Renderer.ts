...
export default class Renderer {
  ...
  constructor(
    ...
    const regl = REGL({
      canvas: canvas,
      // profile: true,
      optionalExtensions: ['OES_texture_float'], // only used for adv hillshading
  ...
  */
  ...
  renderTileDiff(
    ...
    let fboTile: Framebuffer2D = regl.framebuffer({
      width: tileSize,
      height: tileSize,
      depth: false,
      colorFormat: 'rgba',
  ...
  renderTileHsAdvanced(
    ...
    console.log("renderTileHsAdvanced run")

    // Add image to the texture and retrieve its texture coordinates.
    const textureBounds = textureManager.addTile(tileDatum.coords, tileDatum.pixelData);

    // add elevation fbo? May be faster than decoding rgba every shadow iteration

    let fboNormals = regl.framebuffer({
      width: tileSize,
      height: tileSize,
      // width: tileSize * 3, // TODO: update when working for single tiles
      // height: tileSize * 3,
      depth: false,
      colorType: "float",
      // colorFormat: 'rgba',
    ...
    // TODO: add as option to plugin. Use zoom to change scale?
    console.log(zoom);
    // let scale = 3.9583; // troll
    // lock to zoom level 3
    // const pixelScale = scale * 2^(6-3);

    let scale = 0.5; // reek
    const pixelScale = scale * 2^(3);
    ...
    this.HsAdvCalcNormals({
      ...
      onePixel: 1 / textureManager.texture.width,
      fbo: fboNormals,
    });

    // let sunDirection: vec3 = vec3.normalize([], [1, 1, 1]);
